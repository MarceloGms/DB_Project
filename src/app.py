...
def create_reply(song_id, parent_comment_id):
    ...
    cur.execute("""SELECT consumer_person_id
                FROM comment 
                WHERE song_ismn = %s
                AND comment_id = %s""", (song_id, (parent_comment_id,)))
    ...
    try:
        ...
        for i in range(12):
            ...
            cur.execute("""
                ...
                ORDER BY TO_CHAR(a.listen_date, 'YYYY-MM') ASC, s.genre ASC""", (year, month))
            ...
            for row in rows:
                result["results"].append({
                    "month": row[0],
                    "genre": row[1],
                    "total songs played": row[2],
                    "total playbacks": row[2] * row[3]
                })
